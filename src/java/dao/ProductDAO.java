...
public class ProductDAO extends DBContext {
    ...
    public ArrayList<Category> getCategory() {
        ...
        String sql = "  select* from Category_HE163397";
        try {
            PreparedStatement ps = connection.prepareStatement(sql);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
    ...
    public void DeleteProduct(int pid) {
        String sql = "  DELETE FROM [Product_HE163397] WHERE product_id =?";
        try {
            PreparedStatement ps = connection.prepareStatement(sql);
            ps.setInt(1, pid);
            ps.executeUpdate();
    ...
    public void UpdateProduct(int id, String name, double price, int cateId, int stock, String createDate, String descri, String img) {
        String sql = "  UPDATE [Product_HE163397]  set [product_name]= ?   ,[price] = ?  ,[stock]= ?,  \n"
    ...
    public void AddProduct(String name, double price, int cateId, int stock, String descri, String img) {
        String sql = "insert into [Product_HE163397] ([product_name],[price],[stock],[category_id],[img],[description],[create_date]) values (?,?,?,?,?,?, getdate())";
        try {
            PreparedStatement ps = connection.prepareStatement(sql);
            ps.setString(1, name);
            ps.setDouble(2, price);
